...
class Form {
  ...
  constructor(idForm, classEditInput) {
    ...
    this.VALIDATIONS = {
      street: {
        messageError: "Por favor ingresa una dirección válida (solo letras, números y caracteres como #, ., -)",
        regExp: /^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s\.\#\-\°]{3,50}$/
      },


      TextOrEmail: {
        messageError: "Please enter a valid text (3-20 alphanumeric characters) or email address",
        regExp: /^([a-zA-ZáéíóúÁÉÍÓÚñÑ0-9]{3,20}|[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*)$/
      },
      ...
      simplepassword: {
        messageError: "La contraseña debe tener mínimo 6 caracteres",
        regExp: /^.{6,}$/
      }
  ...
  validateInputs(input) {
    ...
    const validation = this.VALIDATIONS[validationType];

    if (!validation) return true; // No hay reglas para este tipo
    ...
    if (!isValid) {
      ...
      if (!spanError) {
        ...
        spanError.innerHTML = validation.messageError;
      }
      return false;
    } else {
